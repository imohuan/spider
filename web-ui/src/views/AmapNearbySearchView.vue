...
// ---- 坐标 ----
const lng = ref('104.55280335554423')
const lat = ref('30.412038984204283')
const keyword = ref('茶語集茶坊(丽都苑店)')
...
function loadAMapScript(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    if (amapSecurityCode.value) {
      (window as any)._AMapSecurityConfig = { securityJsCode: amapSecurityCode.value }
    }
...
function addMarkers(pois: any[]) {
  ...
  pois.forEach((poi, i) => {
    const m = new AMap.Marker({
      ...
      label: {
        content:
          '<span style="background:#fff;color:#3b82f6;padding:1px 7px;' +
          'border-radius:10px;font-size:11px;font-weight:700;' +
          'border:1.5px solid #3b82f6;box-shadow:0 1px 4px rgba(0,0,0,.15);">' + (i + 1) + '</span>',
...
onMounted(async () => {
  try {
    ...
    raw.forEach((item: any) => { map[item.key] = item.value })
    amapKey.value = map['amap_key'] || ''
    amapSecurityCode.value = map['amap_security_code'] || ''

    if (!amapKey.value) {
      loadError.value = '请先在「配置 → 地图」中填写高德 API Key 和安全密钥'
      loading.value = false
      triggerNotify('高德 API Key 未配置', 'error', '请先配置')
      return
    }
...
<template>
  <div class="flex w-full overflow-hidden" style="height: calc(100% + var(--spacing-margin) * 2); margin: calc(var(--spacing-margin) * -1)">
    <!-- ── 侧边栏 ── -->
