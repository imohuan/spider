...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      ...
      <AxButton icon="add" @click="openAdd" size="lg">添加预设</AxButton>
    </div>
    ...
    <div class="grid gap-3">
      <div v-for="p in presets" :key="p.id"
        ...
        <div class="flex items-center gap-2 shrink-0">
          <AxSwitch :model-value="p.enabled" size="xs" @update:model-value="toggle(p.id)" />
          <AxButton variant="ghost" size="icon-lg" icon="edit" @click="openEdit(p)" />
          <AxButton variant="ghost" size="icon-lg" icon="delete" @click="remove(p.id)" />
        </div>
      </div>
    </div>
    ...
    <AxDialog v-model="dialogOpen" :title="editing.id ? '编辑预设' : '添加预设'" icon="cookie">
      <template #default>
        <div class="space-y-3">
          <AxInput v-model="editing.name" size="lg" placeholder="预设名称（如：58同城-已登录）" />
          <AxInput v-model="editing.domain" size="lg" placeholder="匹配域名（如：jianyangshi.58.com）" />
          <AxInput v-model="editing.cookies_json" placeholder="EditThisCookie 导出的 JSON（粘贴到这里）" multiline :rows="8" />
